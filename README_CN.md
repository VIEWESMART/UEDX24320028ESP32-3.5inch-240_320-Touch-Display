...
5. 编辑示例中的`esp_panel_board_supported_conf.h`文件：
    * 启用宏定义: 将#define ESP_PANEL_BOARD_DEFAULT_USE_SUPPORTED  `(0)`改为`(1)`
    * 取消注释屏幕的型号定义:翻到下方找到 `// #define BOARD_VIEWE_UEDX24320035E_WB_A`取消注释
6. 工具配置（S3）：
    #### ESP32-S3
    | Setting                               | Value                                 |
    | :-------------------------------: | :-------------------------------: |
